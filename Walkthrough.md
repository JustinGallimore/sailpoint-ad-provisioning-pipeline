...
This document walks through every step of the Phase 2 lab build. Each screenshot is embedded inline with an explanation of what is happening, why it matters, and how it connects to real enterprise IAM work. This is not just a configuration guide. It is a record of how a production-grade identity provisioning pipeline is built, tested, and troubleshot from scratch.
...
![New VM Wizard](Screenshots/01_vmware_new_vm_wizard.png)
...
![Custom Hardware Wizard](Screenshots/02_vmware_custom_hardware_wizard.png)
...
![ISO Selection](Screenshots/03_vmware_iso_selection.png)
...
The VA was migrated from VirtualBox to VMware by converting the VDI disk to VMDK format using VBoxManage, creating a new VMware VM pointing to the converted disk, and setting bridged networking to the Intel I226-V adapter. After migration the VA picked up IP 192.168.50.211 and ping tests confirmed 0% packet loss to DC01 at 192.168.50.10.
...
After resolving the VA migration, the LabUsers OU structure is confirmed in Active Directory. Engineering, Finance, HR, IT, and Marketing sub-OUs are all present and ready to receive provisioned accounts.
